...
v7.0 Deep Expansion — 2026-03-23
Changes from v5.5 (ALL ADDITIVE — zero modification of existing D01-D12):
- D13-D25: 13 new Deep Stations covering previously surface-only areas
- Total Deep Stations: 12 → 25 (96 → ~180 sub-checks)
- Coverage: Estimates, POs, Recurring, Fixed Assets, RevRec, Time, Tax, Budgets, Dimensions, Workflows, Custom Fields, Reconciliation, AI
- Learning Log framework: every sweep generates structured JSON + learnings
"""
...
DEEP_STATIONS = [
    ...
    {
        "id": "D13",
        "name": "Estimates & Progress Invoicing",
        "category": "Sales Pipeline",
        "tier": "deep",
        "route": "/app/estimates",
        "description": "Estimates exist with realistic amounts, linked to customers and invoices, progress invoicing used where applicable",
        "what_to_check": [
            "Estimate count (target 5-15 for healthy demo)",
            "Amount range realistic for sector",
            "Customer names match D05 customer list",
            "Conversion rate: estimates converted to invoices",
            "Progress invoicing configured",
            "No placeholder names or Lorem text",
            "Expiration dates set and not all expired",
        ],
        "drill_in": [
            "Open 2-3 estimates to verify line items are realistic",
            "Check if any estimate has been converted to invoice — follow the chain",
            "Look for progress invoicing: partial billing against a single estimate",
        ],
        "sub_checks": [
            "D13.1 — Estimate Volume: How many estimates exist? A healthy demo has 5-15 estimates. Zero estimates means the sales pipeline story is missing entirely — SEs cannot demo 'quote-to-cash' workflow. Over 30 suggests mass-import clutter.",
            "D13.2 — Amount Realism: Open 3 estimates and check amounts. Construction estimates should be $10K-$500K. Services should be $1K-$50K. Amounts under $100 or over $10M look fabricated and undermine demo credibility.",
            "D13.3 — Customer Linkage: Do the customers on estimates match the customer list from D05? An estimate for 'John Doe' when D05 shows no such customer indicates orphaned data. Cross-reference at least 3 customer names.",
            "D13.4 — Conversion Rate: What percentage of estimates have been converted to invoices? If 0% conversion, the pipeline story is dead — estimates exist but never became revenue. Ideal is 30-60% converted, showing a realistic win rate.",
            "D13.5 — Progress Invoicing: For Construction/Professional Services, check if any estimate uses progress invoicing (partial billing). This is a key Advanced feature that SEs demo frequently. Look for estimates with 'Progress' column or partial invoice history.",
            "D13.6 — Naming Quality: Scan estimate titles/descriptions. 'Estimate #1', 'Test', or blank descriptions destroy realism. Each estimate should have a descriptive title that tells the business story (e.g., 'Kitchen Renovation - Phase 1', 'Annual IT Consulting').",
            "D13.7 — Expiration Dates: Check if estimates have expiration dates set. All-expired estimates (100% past due) make the business look abandoned. At least 30% should have future or recent dates.",
            "D13.8 — Acceptance Status: What is the distribution of accepted/pending/rejected? All-pending with zero accepted is unrealistic. A healthy mix shows 40-60% accepted, 20-30% pending, 10-20% rejected/expired.",
        ],
        "cross_refs": ["D05", "D09"],
        "auto_fix": True,
        "fix_actions": [
            "Rename placeholder estimate titles with sector-appropriate descriptions",
            "Update expiration dates on expired estimates to 30-60 days from today",
            "DO NOT create new estimates from scratch — only fix naming and dates on existing ones",
        ],
    },
    {
        "id": "D14",
        ...
        "description": "POs exist, linked to bills and vendors, open vs closed ratio healthy, amounts realistic",
        "what_to_check": [
            "PO count (target 5-15 for healthy demo)",
            "PO-to-Bill linkage percentage",
            "Vendor names match D06 vendor list",
            "Open vs closed PO status ratio",
            "Amount distribution realistic for sector",
            "Product/service items match D08",
            "No duplicate POs",
        ],
        "drill_in": [
            "Open 2-3 POs to verify line items and vendor details",
            "Check if any PO has been linked to a bill — follow the procurement chain",
            "Look for POs with status 'Open' that are 90+ days old — they look abandoned",
        ],
        "sub_checks": [
            "D14.1 — PO Volume: How many purchase orders exist? A healthy demo has 5-15 POs. Zero POs means the procurement story is absent — SEs cannot demo the PO→Bill→Payment workflow. Construction demos especially need POs for materials and subcontractor work.",
            "D14.2 — PO→Bill Linkage: What percentage of POs have been converted to or linked with bills? If 0% linkage, the procurement chain is broken — POs exist in isolation with no downstream accounting. Ideal is 40-70% linked, showing active procurement.",
            "D14.3 — Vendor Consistency: Do the vendors on POs match the vendor list from D06? POs for vendors that don't exist in the vendor list indicate orphaned procurement data. Cross-reference at least 3 vendor names between POs and D06.",
            "D14.4 — Status Distribution: What is the ratio of Open vs Closed POs? All-open POs suggest nothing ever gets fulfilled. All-closed means no active procurement. Target: 30-50% open (active orders), 50-70% closed (completed procurement cycle).",
            "D14.5 — Amount Realism: Check PO amounts against sector norms. Construction POs for materials should be $5K-$200K. Service company POs for software/supplies should be $500-$20K. A $0.01 PO or $50M PO destroys credibility.",
            "D14.6 — Item Consistency: Do the products/services on PO line items match the items in D08? POs referencing items not in the product catalog indicate data integrity issues. Spot-check 3 PO line items against D08.",
            "D14.7 — Duplicate Detection: Scan for POs with identical vendors + amounts + dates. Duplicate POs suggest mass-import artifacts or data corruption. More than 2 exact duplicates is a red flag.",
        ],
        "cross_refs": ["D06", "D08"],
        "auto_fix": False,
        "fix_actions": [
            "DO NOT modify POs automatically — complex data relationships with bills and payments",
            "Register PO-to-Bill linkage gaps as P2 (weakens procurement demo story)",
            "Register zero POs as P1 (blocks procurement workflow demo entirely)",
        ],
    },
    {
        "id": "D15",
        ...
        "tier": "deep",
        "route": "/app/recurring",
        "description": "Recurring templates exist with diverse types and realistic intervals, linked to vendors/customers",
        "what_to_check": [
            "Template count (target 5-10)",
            "Interval diversity (monthly/weekly/quarterly)",
            "Type diversity (bill/invoice/expense)",
            "Vendor/customer linkage",
            "Next occurrence date not in distant past",
            "Amount realism",
            "Template names descriptive (no placeholders)",
        ],
        "drill_in": [
            "Open 2-3 recurring templates to verify configuration completeness",
            "Check if any templates have actually generated transactions (execution history)",
            "Verify intervals make business sense (rent=monthly, not daily)",
        ],
        "sub_checks": [
            "D15.1 — Template Volume: How many recurring transaction templates exist? Target is 5-10. Zero templates means the business has no operational rhythm — no rent, no subscriptions, no regular billing. This kills the 'operational maturity' narrative that SEs use to show QBO value.",
            "D15.2 — Interval Diversity: Are all templates set to the same interval? If all 8 templates are 'Monthly', the setup looks templated and unrealistic. A real business has monthly (rent, subscriptions), weekly (payroll, supplies), and quarterly (insurance, tax estimates) intervals.",
            "D15.3 — Type Diversity: What transaction types are represented? Having only recurring bills is one-dimensional. A healthy mix includes: recurring invoices (showing predictable revenue), recurring bills (showing predictable expenses), and recurring expenses (showing operational costs).",
            "D15.4 — Vendor/Customer Linkage: Do the vendors and customers on recurring templates match entities from D05/D06? A recurring bill to 'Vendor X' that doesn't exist in the vendor list is an orphaned reference.",
            "D15.5 — Schedule Currency: When is the next occurrence date for each template? If ALL templates show next dates 6+ months in the past, the recurring system looks abandoned. At least 50% should have next dates within the current or next month.",
            "D15.6 — Amount Realism: Are recurring amounts realistic? Monthly rent of $0.01 or $5M is absurd. Monthly software subscriptions should be $50-$500. Monthly rent for a small business should be $1K-$15K. Amounts should match what the vendor/category would actually charge.",
            "D15.7 — Naming Quality: Do template names describe the actual recurring transaction? 'Recurring Bill #1' or 'Template' are placeholders. Good names: 'Monthly Office Rent - WeWork', 'Weekly Janitorial Service', 'Quarterly Insurance Premium'.",
        ],
        "cross_refs": ["D05", "D06"],
        "auto_fix": True,
        "fix_actions": [
    ...
    },
    {
        ...
        "description": "Fixed assets exist with depreciation schedules running, values realistic for sector, disposed assets tracked",
        "what_to_check": [
            "Asset count (target 5-15 for healthy demo)",
            "Depreciation method (SL/MACRS)",
            "Original cost range realistic",
            "Accumulated depreciation present",
            "Asset categories (vehicles/equipment/furniture)",
            "Acquisition dates spread over time",
            "No negative depreciation values",
        ],
        "drill_in": [
            "Open 2-3 assets to verify depreciation schedules are running",
            "Check if any assets have been disposed — shows full lifecycle",
            "Verify net book value = original cost - accumulated depreciation",
        ],
        "sub_checks": [
            "D16.1 — Asset Inventory: How many fixed assets are registered? A healthy demo for Construction has 5-15 assets (trucks, excavators, office equipment). Services companies have 3-8 (computers, furniture, vehicles). Zero assets means the capital investment story is missing.",
            "D16.2 — Depreciation Method: What depreciation method is configured? Straight-line (SL) is most common for demos. MACRS shows tax sophistication. If no depreciation method is set, the asset is just a static number with no accounting lifecycle — this defeats the purpose of the Fixed Assets feature.",
            "D16.3 — Cost Realism: Are original cost values realistic? A construction truck at $500 or office furniture at $2M looks fabricated. Vehicles: $25K-$80K. Equipment: $5K-$500K. Computers: $1K-$5K. Furniture: $500-$10K.",
            "D16.4 — Depreciation Progress: Is accumulated depreciation greater than $0? If all assets show $0 accumulated depreciation, either they were just added (check acquisition dates) or depreciation isn't running. Assets acquired 1+ year ago MUST show depreciation progress.",
            "D16.5 — Category Diversity: Are assets spread across multiple categories? All assets being 'Equipment' is one-dimensional. A realistic business has vehicles, equipment, furniture, leasehold improvements, and possibly software/intangibles.",
            "D16.6 — Acquisition Date Spread: Are all assets acquired on the same date? That screams bulk import. Realistic acquisition dates are spread over 1-5 years, showing organic growth of the asset base.",
            "D16.7 — Disposal Tracking: Has any asset been disposed of? Disposal shows the full asset lifecycle (acquire → depreciate → dispose). At least 1 disposed asset in a mature demo demonstrates this workflow to prospects.",
            "D16.8 — Balance Sheet Cross-Check: Does the total net book value of all fixed assets approximately match the 'Fixed Assets' line on the Balance Sheet (D03)? A large discrepancy indicates the fixed asset register is disconnected from the general ledger.",
        ],
        "cross_refs": ["D03", "D11"],
        "auto_fix": False,
        "fix_actions": [
            "DO NOT create or modify fixed assets automatically — depreciation schedules affect financial statements",
            "Register zero assets as P2 (missing feature demo, not a data integrity issue)",
            "Register broken depreciation (0 accumulated on old assets) as P1 (accounting inaccuracy)",
        ],
    },
    {
        "id": "D17",
        ...
        "description": "RevRec schedules exist with deferred revenue tracked, recognition periods realistic (Advanced only)",
        "what_to_check": [
            "Schedule count",
            "Recognition method configured",
            "Deferred revenue balance matches BS",
            "Period distribution not all same month",
            "Linked invoices exist",
            "Start/end dates realistic",
        ],
        "drill_in": [
            "Open 1-2 schedules to verify recognition timeline",
            "Check deferred revenue account on Balance Sheet (D03) for consistency",
            "Verify at least one schedule has partial recognition (not 0% or 100%)",
        ],
        "sub_checks": [
            "D17.1 — Feature Availability: Is Revenue Recognition accessible? This is an Advanced-only feature. If the route returns 404 or 'Upgrade required', note as SKIP (not a defect — tier limitation). Do NOT mark this as a failure on Plus/Simple Start environments.",
            "D17.2 — Schedule Volume: How many RevRec schedules exist? For Advanced demos, target is 3-8. Zero schedules means the most sophisticated accounting feature of Advanced is unused — SEs cannot demo ASC 606 compliance or deferred revenue workflows.",
            "D17.3 — Recognition Method: What recognition method is configured (straight-line, milestone, usage-based)? The method should match the business model. SaaS/subscription → straight-line over contract term. Construction → percentage of completion. If no method is set, RevRec is configured but not functional.",
            "D17.4 — Deferred Revenue Consistency: Does the deferred revenue balance in RevRec match the 'Deferred Revenue' liability on the Balance Sheet (D03)? A mismatch means the RevRec module and GL are out of sync — this is a data integrity issue that an accountant would catch immediately.",
            "D17.5 — Period Distribution: Are all recognition events in the same month? If all revenue recognizes in January, the schedule is unrealistic. Revenue should be spread across multiple months matching the service delivery period.",
            "D17.6 — Invoice Linkage: Are RevRec schedules linked to actual invoices (D05)? Orphaned schedules with no source invoice show incomplete setup. At least 70% of schedules should trace back to a real invoice.",
            "D17.7 — Partial Recognition: Is there at least one schedule showing partial recognition (e.g., 40% recognized, 60% deferred)? If all schedules are 0% or 100%, the time-based recognition story is invisible — the demo cannot show revenue being recognized over time.",
        ],
        "cross_refs": ["D02", "D03", "D05"],
        "auto_fix": False,
        "fix_actions": [
            "DO NOT modify RevRec schedules — complex accounting with ASC 606 compliance implications",
            "Register zero schedules as P2 on Advanced environments (missing feature demo)",
            "Register deferred revenue mismatch as P1 (GL integrity issue)",
            "SKIP entirely if feature not available (not Advanced tier)",
        ],
    },
    {
        "id": "D18",
        ...
        "tier": "deep",
        "route": "/app/time",
        "description": "Time entries exist linked to employees and projects, billable hours tracked, hours realistic",
        "what_to_check": [
            "Total time entries (target 20+ for healthy demo)",
            "Employee coverage (% of employees with entries)",
            "Project coverage (% of projects with time)",
            "Billable vs non-billable ratio",
            "Hours per week realism (not 200h/week)",
            "Description quality (not empty/placeholder)",
            "Date spread (not all same day)",
        ],
        "drill_in": [
            "Open weekly view and verify time entries are spread across days",
            "Check if billable entries are linked to customers for invoicing",
            "Verify at least 2 different employees have logged time",
        ],
        "sub_checks": [
            "D18.1 — Entry Volume: How many time entries exist? A healthy demo has 20+ entries across multiple weeks. Zero entries means the time tracking story is dead — SEs cannot demo billable hour workflows, timesheet approvals, or time-to-invoice conversion.",
            "D18.2 — Employee Coverage: What percentage of employees (D07) have at least one time entry? If only 1 of 10 employees has time entries, the feature looks like it was tested by one person, not adopted by the team. Target: 50%+ of employees with entries.",
            "D18.3 — Project Coverage: What percentage of projects (D09) have time entries linked? Time tracking powers project profitability. If projects exist but have zero time logged, the project costing story collapses. Target: 60%+ of active projects with time data.",
            "D18.4 — Billable Ratio: What is the billable vs non-billable split? For professional services, 60-80% billable is realistic. For construction, 70-90% billable. If 0% billable, the 'bill time to clients' workflow cannot be demoed. If 100% billable, the categorization looks artificial.",
            "D18.5 — Hours Realism: Check weekly totals per employee. More than 60 hours/week for any employee looks unrealistic. More than 100 hours/week is impossible and indicates data fabrication. Zero-hour entries (0.00h) are waste records.",
            "D18.6 — Description Quality: Open 5 time entries. Do they have meaningful descriptions? 'Work', 'Time', or empty descriptions provide zero context. Good descriptions: 'Foundation inspection - Lot 42', 'Client meeting - Q1 review', 'API integration debugging'.",
            "D18.7 — Date Distribution: Are all time entries on the same day or same week? Realistic time data spans multiple weeks. If all entries are from a single day, it was clearly a bulk-creation exercise, not organic time tracking.",
            "D18.8 — Rate Linkage: Are billable time entries associated with billing rates? If billable hours exist but no rate is set, the time-to-invoice conversion will show $0 — defeating the entire purpose of billable time tracking.",
        ],
        "cross_refs": ["D07", "D09"],
        "auto_fix": False,
        "fix_actions": [
            "DO NOT create time entries automatically — time data requires realistic employee-project-date combinations",
            "Register zero time entries as P2 (blocks time tracking demo)",
            "Register zero billable entries as P1 (blocks billable hour workflow demo)",
        ],
    },
    {
        "id": "D19",
        ...
        "tier": "deep",
        "route": "/app/salestax",
        "description": "Sales tax configured with agencies, filing status current, taxable items properly categorized",
        "what_to_check": [
            "Tax agency count",
            "Filing frequency configured",
            "Tax rate set",
            "Nexus states (multi-state?)",
            "Overdue filings count",
            "Taxable item percentage in product list",
            "Tax liability balance",
        ],
        "drill_in": [
            "Check the Sales Tax Center for overdue filings or setup prompts",
            "Verify at least one tax agency is registered with a filing frequency",
            "Cross-check: do invoices (D05) show tax being collected?",
        ],
        "sub_checks": [
            "D19.1 — Tax Setup Status: Is sales tax set up at all? Navigate to /app/salestax. If it shows a setup wizard or 'Set up sales tax', the feature is unconfigured. An unconfigured tax module is acceptable for Non-Profits (often tax-exempt) but is a gap for Construction and Services companies that collect tax.",
            "D19.2 — Agency Registration: How many tax agencies are registered? A single-state business needs at least 1 (state tax agency). Multi-state businesses need one per nexus state. Zero agencies after setup means tax is configured but empty — filings cannot be demonstrated.",
            "D19.3 — Filing Frequency: Is the filing frequency set (monthly/quarterly/annually)? An unset frequency means the filing workflow cannot be demoed. Monthly filers should show 12 periods per year in the filing history.",
            "D19.4 — Tax Rate Verification: Is the tax rate configured and reasonable? US state sales tax ranges from 0% (OR, NH) to 10%+ (TN, combined). A 50% tax rate or 0.001% rate indicates misconfiguration.",
            "D19.5 — Overdue Filings: Are there overdue tax filings? While 1-2 overdue is realistic, 12+ overdue filings make the business look non-compliant. Check the filing history for a mix of filed and upcoming periods.",
            "D19.6 — Taxable Item Coverage: Cross-reference with D08 — what percentage of products/services are marked as taxable? For a retail/construction business, 60-80% of items should be taxable. If 0% of items are taxable, invoices will never collect tax, making the tax module pointless.",
            "D19.7 — Tax Liability Balance: Is there a sales tax liability balance? Check the Balance Sheet (D03) for 'Sales Tax Payable'. A business collecting tax should have a liability balance matching uncollected/unfiled periods. Zero liability with active tax collection indicates an accounting issue.",
            "D19.8 — Exemption Certificates: Do any customers have tax exemption certificates on file? For B2B demos, this is a common SE talking point. Check 2-3 customer records from D05 for exemption status.",
        ],
        "cross_refs": ["D05", "D08", "D12"],
        "auto_fix": False,
        "fix_actions": [
            "DO NOT modify tax settings — tax configuration affects compliance and financial accuracy",
            "Register unconfigured tax as P2 (missing feature demo, not data integrity)",
            "Register overdue filings (12+) as P2 (cosmetic, not blocking)",
            "NEVER change tax rates or agency settings — these have legal implications",
        ],
    },
    {
        "id": "D20",
        "name": "Budgets & Variance Analysis",
        "category": "Planning & Control",
        "tier": "deep",
        "route": "/app/budgets",
        "description": "Budget exists with monthly allocation, BvA report generates meaningful variance data",
        "what_to_check": [
            "Budget count (at least 1)",
            "Budget type (P&L/BS)",
            "Period coverage (full year?)",
            "Pre-filled from actuals",
            "BvA report renders with data",
            "Monthly allocation pattern",
        ],
        "drill_in": [
            "Open the budget and verify monthly amounts are populated",
            "Run Budget vs Actuals report and verify it shows meaningful variance",
            "Check if budget categories match COA accounts (D11)",
        ],
        "sub_checks": [
            "D20.1 — Budget Existence: Does at least one budget exist? Zero budgets means the entire planning and variance analysis workflow is unavailable. SEs frequently demo BvA reports to show financial planning maturity — no budget means no BvA.",
            "D20.2 — Budget Type: Is the budget a P&L budget (most common) or Balance Sheet budget? P&L budgets are the standard for SMB demos. Having both P&L and BS budgets shows advanced planning capability.",
            "D20.3 — Period Coverage: Does the budget cover the full fiscal year? A budget covering only Q1 looks incomplete. Check that all 12 months have allocations. Partial budgets (3-6 months) are acceptable only if labeled as such.",
            "D20.4 — Pre-filled vs Empty: Was the budget pre-filled from prior year actuals? Pre-filled budgets show realistic numbers immediately. A budget with $0 in every line item is useless for BvA analysis.",
            "D20.5 — BvA Report Validation: Navigate to Reports > Budget vs Actuals. Does the report render with actual data showing variance ($ and %)? If the report shows all zeroes or 'No data', the budget is not connected to actual GL data. This is THE key deliverable of the budget feature.",
            "D20.6 — Variance Quality: In the BvA report, are variances meaningful? All categories showing 0% variance means budget exactly equals actuals (copy-paste, not planning). Realistic variance is 5-30% for most line items. Some over-budget (red) and some under-budget (green) creates a compelling visual.",
            "D20.7 — Category Alignment: Do budget categories match the Chart of Accounts (D11)? Budget line items should map to actual GL accounts. Budget categories that don't exist in the COA indicate a disconnected planning process.",
            "D20.8 — Monthly Allocation Pattern: Is the monthly allocation flat (same amount every month) or shaped? Flat allocation is acceptable but boring. Seasonal patterns (higher in Q4, lower in Q1) show business acumen. At minimum, verify amounts aren't identical across all 12 months.",
        ],
        "cross_refs": ["D02", "D11"],
        "auto_fix": True,
        "fix_actions": [
            "IF NO BUDGET EXISTS: Create a P&L budget from actuals (QBO offers 'Pre-fill from prior year')",
            "Use the 'Pre-fill data from' option to auto-populate from last year's actual P&L",
            "Name the budget descriptively: 'FY2026 Operating Budget' not 'Budget 1'",
            "After creation, verify BvA report generates data before marking as fixed",
        ],
    },
    {
        "id": "D21",
        "name": "Classes, Locations & Tags (Dimensional Analysis)",
        "category": "Analytical Depth",
        "tier": "deep",
        "route": "/app/class + /app/locations + Tags panel",
        "description": "Dimensions configured and actively USED on transactions, powering segmented reporting",
        "what_to_check": [
            "Class count and naming",
            "Location count and naming",
            "Tag group count",
            "Transaction coverage % (txns with class assigned)",
            "P&L by Class report has data",
            "P&L by Location report has data",
            "Orphan dimensions (0 transactions)",
        ],
        "drill_in": [
            "Navigate to /app/class and count active classes",
            "Run P&L by Class report — verify each class column has data",
            "Spot-check 3 invoices/expenses to see if dimensions are assigned at line level",
        ],
        "sub_checks": [
            "D21.1 — Class Setup: Navigate to /app/class. How many classes exist? Target is 3-8 for a healthy demo. Zero classes means no departmental or project-based reporting — P&L by Class will be empty. Class names should be sector-appropriate (Construction: Earthwork, Concrete, Electrical; Services: Consulting, Support, Training).",
            "D21.2 — Location Setup: Navigate to /app/locations. How many locations exist? For multi-location businesses, 2-5 locations is realistic. Zero locations is acceptable for single-office businesses but limits reporting dimensionality.",
            "D21.3 — Tag Configuration: Check Tags (Settings > Tags or in transaction forms). Are tag groups defined? Tags provide ad-hoc categorization beyond classes/locations. Even 1-2 tag groups (e.g., 'Region', 'Priority') add analytical depth to the demo.",
            "D21.4 — Transaction Coverage: This is the CRITICAL check. Open 5 recent transactions (mix of invoices and expenses). How many have a class assigned? If 0 of 5 have classes, the dimensional data is hollow — classes exist as labels but aren't used. Target: 60%+ of transactions should have class assignments.",
            "D21.5 — P&L by Class Report: Run the 'Profit and Loss by Class' report. Does each class column show both income AND expense data? Columns with all $0 indicate the class was created but never used on transactions. This is the #1 hidden data integrity issue — it's invisible until someone runs this report.",
            "D21.6 — P&L by Location Report: If locations exist, run 'Profit and Loss by Location'. Same validation as D21.5 — each location should have transaction data. An 'Unspecified' column larger than 50% means most transactions lack location tags.",
            "D21.7 — Orphan Dimension Detection: Are there classes or locations with ZERO transactions? These are dead dimensions that clutter dropdowns and confuse users. More than 30% orphaned dimensions indicates mass-creation without follow-through.",
            "D21.8 — Tag Usage on Transactions: If tags exist, spot-check 3 invoices. Are tags actually applied? Tags that exist in settings but never appear on transactions are invisible to reports and demos.",
        ],
        "cross_refs": ["D02", "D05", "D06", "D11"],
        "auto_fix": False,
        "fix_actions": [
            "DO NOT assign dimensions to transactions automatically — class/location assignment is business-context dependent",
            "Register zero classes as P2 (limits reporting but not a data integrity issue)",
            "Register empty P&L by Class (all $0 columns) as P1 (the classification data is hollow — blocks segmented reporting demos)",
            "Register >50% 'Unspecified' in location report as P2 (weak location adoption)",
        ],
    },
    {
        "id": "D22",
        ...
        "tier": "deep",
        "route": "/app/workflows",
        "description": "Automation workflows configured and active, showing QBO Advanced value proposition",
        "what_to_check": [
            "Workflow count (target 2-5)",
            "Active vs inactive status",
            "Trigger type diversity",
            "Action type diversity",
            "Execution history (have they fired?)",
            "Meaningful names (not 'Workflow 1')",
        ],
        "drill_in": [
            "Open 1-2 workflows to verify trigger + action configuration is complete",
            "Check execution/run history for recent activity",
            "Verify workflows reference real entities (customers, accounts, amounts)",
        ],
        "sub_checks": [
            "D22.1 — Feature Availability: Is Workflows accessible? This is primarily an Advanced feature. If the route returns 404 or requires upgrade, note as SKIP. Do NOT mark as failure on Plus/Simple Start environments.",
            "D22.2 — Workflow Volume: How many workflows exist? Target is 2-5 for a healthy demo. Zero workflows on an Advanced environment means the automation value proposition is completely undemonstrated — this is one of the top reasons customers upgrade to Advanced.",
            "D22.3 — Active Status: What percentage of workflows are active vs inactive/draft? All-inactive workflows suggest the feature was explored but abandoned. At least 60% should be active, showing the business relies on automation.",
            "D22.4 — Trigger Diversity: What trigger types are used? Having 3 workflows all triggered by 'Invoice overdue' is monotone. A rich demo shows diverse triggers: invoice overdue, expense threshold exceeded, payment received, estimate accepted. Each trigger tells a different automation story.",
            "D22.5 — Action Diversity: What actions do workflows perform? Email notifications are basic. A mature setup includes: send email, create task, request approval, update field, notify team. Multiple action types demonstrate the breadth of QBO automation.",
            "D22.6 — Execution History: Have any workflows actually executed? Check the run/execution log. Workflows that have never fired are theoretical — they show configuration but not value. At least 1 workflow should have execution history showing it processed real transactions.",
            "D22.7 — Naming Quality: Are workflow names descriptive? 'Workflow 1', 'Test', or 'New Workflow' are placeholders. Good names: 'Notify Manager: Invoice >$10K Overdue', 'Auto-Email: Payment Received Thank You', 'Alert: Expense Over Budget'.",
        ],
        "cross_refs": ["D05", "D06"],
        "auto_fix": True,
        "fix_actions": [
            "IF EMPTY on Advanced: Create 2-3 demo workflows:",
            "  1. 'Overdue Invoice Alert' — Trigger: Invoice overdue by 7 days, Action: Send email to company owner",
            "  2. 'Large Expense Notification' — Trigger: Expense created > $5,000, Action: Notify manager",
            "  3. 'Payment Thank You' — Trigger: Payment received, Action: Send thank-you email to customer",
            "Set all workflows to ACTIVE status after creation",
            "IMPORTANT: Use real customer/vendor names from the environment (D05/D06) in workflow conditions",
        ],
    },
    {
        "id": "D23",
        "name": "Custom Fields & Form Templates",
        "category": "Customization",
        "tier": "deep",
        "route": "/app/settings?panel=sales (Custom Fields section) + Custom Form Styles",
        "description": "Custom fields defined and used on forms, templates customized with branding",
        "what_to_check": [
            "Custom field count (target 3-5)",
            "Field types used (text/dropdown/number)",
            "Fields applied to forms",
            "Custom form template exists",
            "Logo on template",
            "Fields populated on actual transactions",
        ],
        "drill_in": [
            "Navigate to Settings > Custom Fields to review field definitions",
            "Open a custom form template to verify branding (logo, colors)",
            "Spot-check 3 invoices to see if custom fields are populated",
        ],
        "sub_checks": [
            "D23.1 — Custom Field Count: How many custom fields are defined? Target is 3-5. Zero custom fields means the platform customization story is absent. SEs demo custom fields to show QBO adapts to any business — 'add a PO Number field to invoices', 'add a Project Code to expenses'.",
            "D23.2 — Field Type Variety: What field types are used? All text fields are basic. A strong setup includes: dropdown (for standardized values), number (for quantities/codes), and text (for free-form). Dropdown fields are especially impressive in demos because they show data governance.",
            "D23.3 — Form Application: Which forms have custom fields applied? Fields defined but not attached to any form are invisible. Check that custom fields appear on invoices (most common demo), estimates, and/or purchase orders.",
            "D23.4 — Custom Form Template: Does a customized form template exist (beyond the default)? Navigate to Custom Form Styles. A branded template with the company logo, custom colors, and footer text shows professional output — this is what prospects see when they imagine sending invoices to THEIR customers.",
            "D23.5 — Logo Presence: Does the custom form template include a company logo? An invoice template without a logo looks generic and unfinished. The logo should match the company identity established in D12.",
            "D23.6 — Field Population Rate: Open 3 invoices that should have custom fields. Are the fields actually populated with data? Custom fields that exist on the form but are always empty provide no demo value — they're just blank boxes on the invoice.",
            "D23.7 — Default Template Setting: Is the custom template set as the default? If the default template is still QBO's generic template, all new transactions will use the unbranded version, undermining the customization story.",
        ],
        "cross_refs": ["D05", "D12"],
        "auto_fix": False,
        "fix_actions": [
            "DO NOT create custom fields automatically — field definitions are business-specific",
            "Register zero custom fields as P3 (nice-to-have, not critical for demo)",
            "Register missing logo on form template as P2 (easy fix, high visual impact)",
            "Note: Custom form templates can be created via Settings > Custom Form Styles if needed",
        ],
    },
    {
        "id": "D24",
        "name": "Reconciliation Health & Bank Rules",
        "category": "Banking Maturity",
        "tier": "deep",
        "route": "/app/reconcile + /app/banking Rules tab",
        "description": "Accounts reconciled recently, bank rules active and useful, matching operational maturity",
        "what_to_check": [
            "Reconciled account count",
            "Last reconcile date per account",
            "Unreconciled period gap",
            "Beginning balance match",
            "Rule count (target 5-10)",
            "Rule trigger patterns",
            "Auto-add status (should be OFF for demo)",
        ],
        "drill_in": [
            "Navigate to /app/reconcile and check reconciliation status for primary checking account",
            "Check bank rules on /app/banking Rules tab — are rules active?",
            "Verify rules reference real vendors/categories from the environment",
        ],
        "sub_checks": [
            "D24.1 — Reconciliation Status: Navigate to /app/reconcile. How many accounts have been reconciled? Zero reconciled accounts means the most fundamental accounting workflow is undemonstrated — reconciliation is Accounting 101 and SEs always show it.",
            "D24.2 — Last Reconcile Date: When was the most recent reconciliation for the primary checking account? If the last reconcile was 6+ months ago, the account looks abandoned. The primary account should be reconciled within the last 1-2 months for the demo to feel current.",
            "D24.3 — Unreconciled Gap: How many months of unreconciled transactions exist? A gap of 3+ months means there's a backlog of unmatched transactions. While 1 month unreconciled is normal (current period), 6+ months of backlog indicates the environment was set up but never maintained.",
            "D24.4 — Beginning Balance Match: Does the beginning balance for reconciliation match the bank statement? A beginning balance mismatch is a CRITICAL reconciliation error that blocks the entire process. If mismatched, reconciliation cannot proceed without adjusting entries.",
            "D24.5 — Bank Rule Volume: Navigate to the Rules tab in Banking. How many bank rules exist? Target is 5-10 rules. Zero rules means the smart categorization story is missing — SEs demo rules to show 'QBO learns your patterns'. More than 30 rules may indicate over-configuration.",
            "D24.6 — Rule Quality: Open 2-3 bank rules. Do they reference real vendors and categories from the environment? A rule that says 'If description contains XYZ then categorize as Account 1' with generic placeholders is useless. Good rules: 'If contains HOME DEPOT → categorize as Building Materials, assign to vendor Home Depot'.",
            "D24.7 — Auto-Add Setting: Are any rules set to 'Auto-add'? For demos, auto-add should generally be OFF — we want transactions in the 'For Review' queue so SEs can demonstrate the review workflow. Auto-add bypasses the review step and removes a key demo moment.",
            "D24.8 — Rule Coverage: What percentage of recurring vendors (from D06) have corresponding bank rules? If the business has 10 regular vendors but only 1 bank rule, the automation story is thin. Target: rules for at least 50% of top recurring vendors.",
        ],
        "cross_refs": ["D04", "D06"],
        "auto_fix": True,
        "fix_actions": [
            "IF NO RULES EXIST: Create 5-8 bank rules based on existing vendors (D06):",
            "  For each top vendor, create a rule: 'If bank text contains [vendor name] → Categorize as [appropriate expense account] + Assign vendor'",
            "  Set all rules to 'Ask' mode (not Auto-add) so transactions appear in Review queue",
            "DO NOT modify reconciliation data — reconciliation affects account balances",
            "Register 6+ month reconciliation gap as P1 (fundamental accounting workflow gap)",
        ],
    },
    {
        "id": "D25",
        "name": "AI Features & Intuit Intelligence",
        "category": "AI & Innovation",
        "tier": "deep",
        "route": "Various (homepage AI panel, /app/business-intelligence, Intuit Assist)",
        "description": "AI features accessible and generating insights, showing Intuit's AI-first strategy",
        "what_to_check": [
            "Intuit Assist responds to queries",
            "AI-categorized bank transactions exist",
            "Financial Summary in Business Feed",
            "Smart Match suggestions in banking",
            "Intuit Intelligence dashboard loads",
            "AI-generated insights present",
        ],
        "drill_in": [
            "Try asking Intuit Assist 'What were my top expenses last month?'",
            "Check Business Feed for AI-generated Financial Summary tiles",
            "Navigate to banking and verify Smart Match suggestions appear",
        ],
        "sub_checks": [
            "D25.1 — Intuit Assist Availability: Is Intuit Assist accessible? Look for the AI chat icon or 'Ask Intuit Assist' prompt. If present, try a simple query: 'What were my top expenses?' or 'Show me overdue invoices'. The response should reference actual data from this environment, not generic answers. This is the headline AI feature that SEs demo at every opportunity.",
            "D25.2 — AI Bank Categorization: Navigate to Banking (D04 cross-ref). Are there transactions with AI-suggested categories (indicated by sparkle/AI icon)? AI categorization shows that QBO's ML models are working on this environment's data. If zero AI suggestions appear, the ML pipeline may not have processed this tenant's data.",
            "D25.3 — Financial Summary: Check the Business Feed on the homepage (D01 cross-ref). Is there an AI-generated 'Financial Summary for [Month]' tile? This is the Finance AI agent's output — it summarizes income trends, expense spikes, and A/R status. If present, click to verify the summary contains real data points (not placeholder text).",
            "D25.4 — Smart Match in Banking: When reviewing bank transactions, does QBO suggest matches to existing transactions (invoices, bills)? Smart Match is the AI-powered matching engine. Check if the 'Match' column shows suggested matches with confidence indicators. Zero suggestions may indicate insufficient transaction data for the ML model.",
            "D25.5 — Intuit Intelligence Dashboard: Navigate to /app/business-intelligence or look for 'Intuit Intelligence' in the sidebar. Does the dashboard load with benchmark data and performance metrics? This feature compares the business against industry peers — it requires sufficient data history to generate meaningful benchmarks.",
            "D25.6 — KPI Scorecard: Check if the KPI Scorecard is accessible (Fall+Winter release feature). Does it show populated metrics across categories (Revenue, Expenses, Profitability, Cash Flow, Growth)? An empty scorecard indicates the AI hasn't processed enough data to generate KPIs.",
            "D25.7 — AI Insight Count: Across all AI touchpoints (homepage, banking, reports), how many distinct AI-generated insights are visible? A healthy Advanced environment should show 5+ AI insights. Zero AI presence on an Advanced environment is a significant gap — AI is Intuit's primary differentiator in 2025-2026.",
            "D25.8 — Conditional Assessment: AI features depend on tier (Advanced gets more), data volume (ML needs history), and feature flags (gradual rollout). If AI features are absent, determine WHY: (a) tier limitation → note as expected, (b) insufficient data → note as environment gap, (c) feature flag not enabled → note as platform limitation. Do not mark AI absence as a defect if the tier doesn't support it.",
        ],
        "cross_refs": ["D01", "D04", "D10"],
        "auto_fix": False,
        "fix_actions": [
            "DO NOT attempt to force AI features — they are platform-generated and depend on ML pipelines",
            "Register zero AI presence on Advanced as P2 (feature gap, not data integrity)",
            "Register broken Intuit Assist (errors on query) as P1 (headline feature broken)",
            "Note: AI features may require 30+ days of transaction history to activate ML models",
            "SKIP AI checks entirely on Simple Start / Essentials tiers where AI features are limited",
        ],
    },
]
...
SURFACE_SCAN = [
    ...
    {"id": "S04", "name": "Expenses", "route": "/app/expenses", "description": "Has expenses registered, entry count"},
    {
        ...
        "name": "Recurring Transactions",
        "route": "/app/recurring",
        "description": "Recurrences configured, entry count. Deep analysis and fixes in D15.",
    },
    {
        "id": "S06",
        "name": "Fixed Assets",
